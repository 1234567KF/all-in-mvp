...
## Browser MCP Agent 与 Playwright 的关系
...
> | 是否纳入管道 | ✅ 是 | ❌ 否，仅临场使用 |
>
> Pipeline 的测试质量不依赖 Browser MCP Agent，它只是一个辅助调试工具。当需要在当前对话中排查失败的测试或人工抽查页面时，使用 Browser MCP Agent。正式的自动化回归测试始终走 Playwright L4/L5。
