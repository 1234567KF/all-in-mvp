...
[ ] Before/after screenshot comparison （如�?CAUSE:VISUAL_CSS�?
...
| `CAUSE:PRD_CONFLICT` | PRD 内部存在矛盾 | 功能需�?A �?B 的验收标准互�?|
| `CAUSE:DESIGN_FLAW` | 架构/Schema 设计缺陷 | 外键关系错误、表结构不支持业务规�?|
| `CAUSE:DESIGN_MISSING` | 架构遗漏 | Schema 缺少必要字段、API 缺少必要端点 |
| `CAUSE:IMPL_ERROR` | 代码实现错误 | 空指针、逻辑错误、类型错�?|
| `CAUSE:IMPL_CONTRACT` | 实现与契约不一�?| 返回的字段名/状态码�?api-contract.yaml 不符 |
| `CAUSE:VISUAL_CSS` | CSS/样式视觉错误 | 颜色错误、布局错乱、元素遮挡、响应式断点失效 |
| `CAUSE:TEST_GAP` | 测试覆盖不足 | 已有测试通过但未覆盖该场�?|
| `CAUSE:ENV_MISMATCH` | 环境差异导致 | 开发环境正常但联调环境出错 |

**视觉�?Bug 特殊要求** �?`CAUSE:VISUAL_CSS` 修复后必须产�?before/after 截图对比�?
...
visual_evidence:
  ...
  description: "按钮颜色从灰�?#9CA3AF 修正为蓝�?#3B82F6"
```
...
| 终止类型 | 条件 | 后续动作 |
|---------|------|---------|
| **正常终止** | 所�?P0/P1 Bug 已修复并验证 | 更新 Bug Report，执行回归测�?|
| **时间终止** | 超过预设时间�? 小时�?| 未修复的 Bug 进入后续迭代 |
| **回归终止** | 修复过程中引入新�?P0 �?Bug | 回退修复，重新评估方�?|
...
- **回归测试目录** �?修复后的回归测试放入 `regression/bug-<编号>-<简�?.test.ts`
- **视觉Bug截图** �?CAUSE:VISUAL_CSS 修复后必须提�?before/after 截图，存�?`regression/screenshots/`
