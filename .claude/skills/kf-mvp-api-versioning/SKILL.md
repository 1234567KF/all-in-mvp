...
description: >-
  ...
  Triggers: API版本, 版本管理, versioning, breaking changes, API演进,
  版本控制, v1 v2, REST版本. Also load when planning API changes.
metadata:
...
| Add new response fields | `{ name: "John", age: 30 }` → add `avatar: "..."` |
| Change field order | `{ a: 1, b: 2 }` → `{ b: 2, a: 1 }` |
| Add new enum values | `status: 'pending' | 'active'` → add `'archived'` |
...
| Change parameter types | `id: string` → `id: number` |
| Change validation rules | `name` required → optional |
...
- **Additive is safe** — New fields don't break existing clients
- **Version length** — Support at least 2 versions simultaneously
- **Sunset date** — Give clients at least 6 months notice
- **Major version** — Only increment for breaking changes
- **Beta** — Use beta versions for experimental features