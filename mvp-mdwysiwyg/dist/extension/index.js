...
// src/extension/services/markdownParser.ts
...
function slugify(text2) {
  return text2.toLowerCase().replace(/[^\w\u4e00-\u9fff]+/g, "-").replace(/^-+|-+$/g, "");
}
...
function extractHeadings(tokens) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === "heading_open") {
      ...
      const text2 = inlineToken?.content || "";
      const id = slugify(text2);
      flat.push({ level, text: text2, id });
    }
  }
...
function buildHeadingTree(flat) {
  ...
  for (const item of flat) {
    ...
    if (stack.length === 0) {
      ...
      stack[stack.length - 1].heading.children.push(heading2);
    }
    stack.push({ heading: heading2, level: item.level });
  }
  return root;
}
...
// src/extension/services/diffEngine.ts
...
function splitIntoSections(content) {
  ...
  const sections = [];
  let current = { headingId: "_preamble", headingText: "", level: 0, lines: [] };
  for (const line of lines) {
    ...
    if (headingMatch) {
      ...
      current = {
        headingId: text2.toLowerCase().replace(/[^\w\u4e00-\u9fff]+/g, "-"),
        headingText: text2,
        level,
        lines: [line]
      };
...
function computeDiff(v0, v1) {
  ...
  for (const s1 of sections1) {
    ...
    const wordDiff2 = diffWords(text0, text1);
    let lineOffset = 0;
    for (const part of wordDiff2) {
      if (part.added) {
        changes.push({
          id: nextChangeId(),
          type: "insert",
...
// src/extension/services/revisionManager.ts
var vscode = __toESM(require("vscode"));
function getRevisionFilePath(mdFilePath) {
...
async function loadRevisionStore(filePath) {
...
async function saveRevisionStore(store) {
  const revPath = getRevisionFilePath(store.filePath);
  const uri = vscode.Uri.file(revPath);
  const json = JSON.stringify(store, null, 2);
  await vscode.workspace.fs.writeFile(uri, Buffer.from(json, "utf-8"));
}
async function setBaseVersion(filePath, content) {
...
async function clearBaseVersion(filePath) {
  const store = await loadRevisionStore(filePath);
...
async function acceptChange(filePath, changeId) {
  const store = await loadRevisionStore(filePath);
...
async function rejectChange(filePath, changeId) {
  const store = await loadRevisionStore(filePath);
  for (const rev of store.revisions) {
    const idx = rev.changes.findIndex((c) => c.id === changeId);
    if (idx !== -1) {
...
async function acceptAllChanges(filePath) {
  const store = await loadRevisionStore(filePath);
...
async function rejectAllChanges(filePath) {
  const store = await loadRevisionStore(filePath);
  store.revisions = store.revisions.map((r) => ({
    ...r,
    status: "rejected",
...
// src/extension/providers/webviewProvider.ts
var MdWysiwygEditorProvider = class _MdWysiwygEditorProvider {
  constructor(context) {
    this.context = context;
  }
  static viewType = "mdWysiwyg.reader";
  static register(context) {
    const provider = new _MdWysiwygEditorProvider(context);
    return vscode2.window.registerCustomEditorProvider(
      _MdWysiwygEditorProvider.viewType,
      provider,
  ...
  async resolveCustomTextEditor(document, webviewPanel, _token) {
    ...
    const sendDocument = async () => {
      ...
      const payload = { content, headings, revisionStore };
    ...
    const changeSubscription = vscode2.workspace.onDidChangeTextDocument((e) => {
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode3.commands.registerCommand("mdWysiwyg.open", async () => {
      const editor = vscode3.window.activeTextEditor;
      if (!editor || !editor.document.fileName.endsWith(".md")) {
        vscode3.window.showWarningMessage("Please open a .md file first");
        return;
      }
  ...
  context.subscriptions.push(
    vscode3.commands.registerCommand("mdWysiwyg.setBaseVersion", () => {
      vscode3.window.showInformationMessage("Set base version: use the toolbar in the WYSIWYG view");
    })
  );
  context.subscriptions.push(
    vscode3.commands.registerCommand("mdWysiwyg.clearBaseVersion", () => {
      vscode3.window.showInformationMessage("Clear base version: use the toolbar in the WYSIWYG view");
    })
  );
  context.subscriptions.push(
    vscode3.commands.registerCommand("mdWysiwyg.toggleView", () => {
      vscode3.window.showInformationMessage("Toggle view: use the toolbar buttons in the WYSIWYG view");
    })
  );
}
function deactivate() {
