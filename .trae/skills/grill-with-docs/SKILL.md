...
> **核心原则**：grill 根据问题类型自动分级，不同级别走不同处理路径。避免低价值问题浪费循环轮次。
...
> 自愈级不占循环轮次，上浮级/终止级首轮直接上浮，仅调试级/协商级走标准循环。

## 强制降级策略

当循环无法在合理时间内达成一致时，自动触发的降级机制：

```
循环硬上限：3 轮
  ↓ 第 3 轮结束仍不一致
  ↓
grill 输出「未决问题清单」+ 按「最简实现原则」推荐方案
  ↓
汇总提交给人类决策
  ↓
人类 24 小时内回复？
  ├── 是 → 按人类决策修正，循环终止
  └── 否 → 自动采纳 grill 推荐方案，记录为「自动裁决」
```
