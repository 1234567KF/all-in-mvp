...
description: >-
  ...
  Triggers: Stage4, 集成协调, 联调编排, 后端合并, 前后端联�? integration
...
## Responsibility 2: 联调问题分类与分�?

当联调发现问题时，Coordinator进行分类和路由：

| 问题类型 | 判定依据 | 分发目标 | 动作 |
|---------|---------|---------|------|
| 契约问题 | 实现�?api-contract.yaml 不一�?| 后端 Agent（修正实现）�?架构专家（修正契约） | 记录�?integration-issues.md |
| 实现问题 | 代码逻辑错误、缺失边界处�?| 对应模块�?Backend Agent | Fix �?单元测试 �?重新联调 |
| 理解偏差 | PRD解读不一致导致的行为差异 | 产品经理 + 对应 Agent | 澄清 �?修正PRD或实�?|
| Mock漂移 | 前端期望与后端响应格式不一�?| Mock Agent + 后端 Agent | Mock同步协议 |
...
Coordinator 读取问题描述 �?分类
    �?
├── 契约问题 �?�?change-request.md �?人类审查�?3接口可自动批准）
...
**Coordinator跟踪�?*（写�?`bug-tracker.md`）：
```markdown
# Bug 跟踪�?

| Bug ID | 标题 | 严重�?| 根因分类 | 状�?| 负责�?| 发现阶段 | 修复验证 |
|--------|------|--------|---------|------|--------|---------|---------|
| BUG-001 | 产品创建返回400 | P0 | CAUSE:IMPL_CONTRACT | FIXED | Debug-1 | 4.2联调 | �?|
| BUG-002 | 溯源码幂等�?| P1 | CAUSE:DESIGN_SCHEMA | IN_PROGRESS | Debug-2 | 4.3集成测试 | - |
```

---

## Responsibility 4: 最终门禁检�?

Stage4交付前的硬性门禁（全部通过才可交付）：

### 4.A 合并门禁
- [ ] 所有模块状态为 DONE（非 BLOCKED/DEFER�?
- [ ] 依赖链闭环——各模块的依赖声明与最终状态一�?
...
- [ ] 所�?API 调用返回格式�?api-contract.yaml 一�?
- [ ] 无未解决�?integration-issues.md 记录
- [ ] 无未解决�?VISUAL_PENDING 标记（所有前端页面视觉已由人类确认）
...
Step 1: 合并前检�?
  ...
  �?检查结果写�?merge-precheck.md

Step 2: 路由聚合
  ├── 收集所有模�?routes.ts 的路由定�?
  ├── 按路径前缀分组
  ├── 挂载到统一 Hono app
  └── 验证中间件链顺序: auth �?body-parser �?router �?error-handler

Step 3: Schema引用解析
  ├── 检查每个模�?schema.ts �?import 路径正确�?
...
- Stage4 合并�? 执行 `drizzle-kit generate` + `drizzle-kit migrate`，生成正式Migration文件纳入版本控制
...
**切换流程**�?
```
1. 检查模块后端状�?�?DONE + Code Review 通过
2. 前端对应页面 �?修改 api.config.ts 中该模块�?baseURL
   user: mock �?user: real
3. 验证该模块的页面功能
4. 通过 �?继续下一个模�?
...
| 触发条件 | 回滚路径 |
|---------|---------|
| 集成测试发现 P0 缺陷 > 3 �?| 回滚�?Stage3 最后一个全量测试通过的检查点 |
| Bug修复引入了新�?P0 缺陷 | 保留所有模块代码，状态回退到检查点 |

**回滚操作流程**�?
1. 读取 `pipeline-state.json` 上一个绿色检查点
2. 对比差异 �?移除检查点后变更（保留Bug修复�?
3. 重置模块状�?�?重新执行 Stage4 门禁

**风险控制**�?
- 回滚是手动触发（需人类确认�?
- 回滚�?Git 备份当前代码
- 回滚后仍无法通过 �?标记"本轮不可交付"，进�?Stage5 复盘

---

# 联调问题记录模板

每个不匹配问题写�?`integration-issues.md`�?
...
| Exception Path | �?80% | 低于80%同上�?0-100%记录WARNING |
