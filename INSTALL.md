# all-in-mvp — Qoder 技能安装与分发指南

多 Agent 并行 MVP 开发流水线技能集 + ultra-cost-effective 极致节能引擎，`npx giget` 一键拉取。
...
```
开发者修改 .qoder/skills/ → git push
                    │
                    ▼
               GitHub 仓库（.gitattributes 自动过滤）
                    │
                    ▼ npx giget 拉取
            用户项目（.qoder/skills/ 项目级）
                    │
                    ▼ 启动 Qoder
            自动识别项目级技能（优先级高于全局）
```

**核心原则**：Qoder 专属，项目级技能不污染全局。
...
> 在当前项目根目录执行。`.qoder/skills/` 直接出现在项目中，启动 Qoder 即可使用。
...
本技能集包含 42 个技能 + ultra-cost-effective 节能引擎，覆盖 MVP 开发全流程：
...
| 模块 | 说明 |
|------|------|
| `ultra-cost-effective` | 七层 Token 节省体系（L1-L7），综合节省 60-90% |
| `project-monitor.cjs` | 项目级全链路 Token 监控（A2A穿透 + 子Agent + 理论节约） |
...
用户通过 tag 锁定版本：
```bash
npx.cmd giget gh:1234567KF/all-in-mvp#v2.9.0 my-project
```
...
A: `.gitattributes` 的 `export-ignore` 规则让 GitHub tarball 自动排除文档/白皮书等文件。只保留 `.qoder/skills/` `install.ps1` `install.sh` `.gitignore`。完整源码在 [GitHub 仓库](https://github.com/1234567KF/all-in-mvp)。

### Q: 技能是全局的还是项目级的？

A: **项目级**。技能在项目根目录 `.qoder/skills/`，Qoder 自动识别且优先级高于全局。每个项目独立拥有，互不影响。
