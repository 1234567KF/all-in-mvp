...
> Skill 文件工程规范——skill-design-expert 在生成 Skill 时按需加载的参考文档。覆盖 frontmatter 格式、命名校验、目录结构等打包层面的约束。
...
```yaml
---
name: your-skill-name          # Required, 1-64 chars, lowercase letters/digits/hyphens
description: What this Skill does + when to trigger. Use imperative mood ("Use when..."), focus on user intent.
...
metadata:                       # Optional, custom extension fields (string→string / string→list)
  pattern: generator
  required-rules: "global-rules, java-kotlin-rules"
  agents:                       #   Optional, list of sub-agent files owned by this Skill
    - my-subagent.md            #     Declares agent→skill mapping for reverse contribution (kb-contribute)
allowed-tools: Bash(git:*) Read # Optional (experimental), space-separated pre-approved tools
---
```

### `metadata.agents` field

When a Skill dispatches work to sub-agents (via `Agent` tool with `subagent_type`), declare those agent files under `metadata.agents`. This enables reverse contribution flow (`kb-contribute`): when an agent file in `.qoder/agents/` needs to flow back to the knowledge base, the `agents` field provides the authoritative agent→skill mapping without relying on directory structure or manifest state.

- **Location**: Inside `metadata` (custom extension fields), not at frontmatter top level
- **Type**: YAML list of filenames (e.g., `["domain-analyzer.md"]`)
- **Scope**: Each filename corresponds to a file under this Skill's `agents/` subdirectory
- **When to add**: Whenever creating a sub-agent that belongs to this Skill
- **Optional**: Skills without sub-agents omit this field entirely
...
- **包含场景关键词**：确保关键场景词出现在 description 中
- **简洁**：几句话到一小段，≤ 1024 字符
...
Skill 内容按受众分层选择语言，不可混用：

| 内容层 | 受众 | 语言 | 理由 |
|--------|------|------|------|
| AI 执行指令（流程/约束/条件分支/Gotchas） | AI 读取 | **英文** | token 效率高，LLM 执行一致性优于中文 |
| 面向用户输出（回复模板/生成文档/错误提示） | 用户阅读 | **用户首选语言** | 减少沟通摩擦 |
| 领域知识（业务规则/内部术语/法规） | AI 理解 | **保持原文** | 强行翻译损失精度 |
| description (L1 metadata) | AI 匹配 | **英文祈使句** | 英文省 token，执行一致性高 |

```markdown
<!-- ❌ 混用 — AI 指令用中文降低执行稳定性 -->
当用户提供代码时，先检查是否包含 SQL 注入漏洞。如果发现问题，用中文告诉用户。

<!-- ✅ 分层清晰 — AI 指令英文，用户输出中文 -->
When the user provides code, check for SQL injection vulnerabilities.
If issues found, explain each vulnerability to the user in Chinese.
```

```yaml
# ✅ description 范例
description: Generate Java CRUD modules from entity definitions — producing Controller, Service, Repository, and DTO layers with validation.
```

> 判断标准：如果这条内容是写给 AI 看的（让它知道怎么做）→ 英文。如果这条内容是 AI 要输出给用户的 → 用户首选语言。
