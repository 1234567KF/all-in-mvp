...
# 用法: powershell -NoProfile -ExecutionPolicy Bypass -File .qoder/scripts/check-stage4-gate.ps1 [-TargetDir .]
# ============================================================
param(
    [string]$TargetDir = ".",
    [switch]$Json
...
if ($failed.Count -gt 0) {
    ...
    Write-Host ""
    Write-Host "修复后重新运行: powershell -NoProfile -ExecutionPolicy Bypass -File .qoder/scripts/check-stage4-gate.ps1" -ForegroundColor Yellow
    exit 1
} else {
    Write-Host ""
