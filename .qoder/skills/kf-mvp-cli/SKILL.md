...
description: >-
  ...
  Triggers: CLIå·¥å…·, å‘½ä»¤è¡? è„šæœ¬, CLIå¼€å? command line, scripts,
  shellè„šæœ¬, bash. Also load when creating developer tools or automation.
metadata:
...
# MVP CLI Tools â€?å‘½ä»¤è¡Œå·¥å…·æŠ€èƒ?
...
1. **Single responsibility** â€?One tool, one purpose
2. **Compose with pipes** â€?Small tools, big results
3. **Clear output** â€?stdout for data, stderr for errors
4. **Fail fast** â€?Check inputs early, fail with clear message
...
export async function seed() {
  ...
  console.log(`âœ?Created ${userList.length} users`);
}
```
...
- **Exit codes** â€?0 = success, non-zero = error
- **Stdout vs stderr** â€?Data goes to stdout, errors to stderr
- **Colors** â€?Respect NO_COLOR env var
- **Progress** â€?Long operations should show progress
- **TTY detection** â€?Colors only when terminal supports it