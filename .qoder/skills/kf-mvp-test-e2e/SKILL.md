...
## E2E 测试味道自检（v2.16 — 每条用例编写后必答）

> 复用 4 种反模式 DNA：测试 Agent 也需要"嗅觉"——写完用例后自问 4 个问题。

1. **代理穿透**：我的 page.goto() URL 是前端端口（5555）还是直连后端（3333）？
   → 必须是前端端口，经过 Vite proxy。直连 = 漏测 axios 拦截器/auth 中间件

2. **交互配额**：这条用例有 page.fill/click/goto 吗？还是全程 api.post()？
   → 浏览器交互占比必须 ≥ 30%（R005）。纯 API 测试放到 Vitest，不要伪装成 E2E

3. **数据加载覆盖**：如果页面有下拉/选择器/表格，我验证了它们的 options/data 非空吗？
   → @headed 模式下必须包含这类断言（R013）。下拉空白是高频漏测点

4. **权限矩阵**：如果系统有角色区分，这条用例覆盖了哪种角色？有预期 403 的用例吗？
   → 每个角色×端点组合至少 1 条用例（R006）。单角色测试 = 权限 bug 全部漏网
