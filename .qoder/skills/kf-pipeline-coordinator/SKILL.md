...
- [ ] Read `task.md` — 提取所有模块清单 + 依赖关系
- [ ] Output「设计清单 vs 分配计划」对照表（写入 decision-log.md）
- [ ] 每轮完成后复核：设计模块 ∩ 已分配模块 == 设计模块
- [ ] 任一 P0 场景 / 模块未纳入分配计划 → **阻断，不得开始调度**

> **P0 红线**：跳过设计文档校验直接调度 → 流程级错误。上一轮 Stage4 因未读 scenarios.md 导致验收范围缩水 62.5%（3/8 场景执行），用户被迫 2 次追问补测。
...
## 阶段完成报告模板（复盘 D-02 — 强制格式）

每阶段结束向用户汇报时，**必须**使用以下模板，禁止自由发挥：

```markdown
## Stage N 完成报告

### 设计 vs 执行对照
| 维度 | 应有（设计文档） | 实有（执行结果） | 对齐率 |
|------|:---:|:---:|:---:|
| 场景总数 | N | N' | N'/N% |
| P0 场景 | N | N' | N'/N% |
| P1 场景 | N | N' | N'/N% |
...
| 后端合并 | ✅/❌ | |
| 前后端联调 | ✅/❌ | |
| 集成测试 | ✅/❌ | Happy N%, Exception N% |
| 设计-执行覆盖 | ✅/❌ | P0 N%, P1 N% |

### 判定
- [ ] COMPLETE（所有维度 100%/≥80%）
- [ ] PARTIAL（部分达标，列出缺口）
- [ ] FAILED（关键维度未达标）
```

> **强制规则**：禁止使用"核心场景已覆盖""主要流程已通过""验证通过✅"等模糊话术。必须输出量化表格。任一维度达成率 < 80% → **禁止标 COMPLETE**。
