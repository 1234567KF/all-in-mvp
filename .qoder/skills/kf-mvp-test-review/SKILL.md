...
description: >-
  ...
  integration test files. Triggers: 测试审查, test review, 测试用例检�?
...
# Test Case Static Reviewer �?测试用例静态审查技�?
...
**Stage2 最后一道防�?*：仅检�?用例写对了没�?，不实际执行（执行在 Stage4）�?

---

# Core Philosophy

Derived from MVP Whitepaper Section 2.7 �?③c 测试用例静态审�?
...
| 文件 | 提供�?| 说明 |
|------|--------|------|
| `integration-tests/modules/` | ③b-1 单模块测试专�?| 每个模块的API集成测试 |
| `integration-tests/scenarios/` | ③b-2 业务条线测试专家 | 跨模块场景测�?|
| `<module>.md` (all) | �?业务领域专家 | 模块验收标准 |
| `PRD.md` | 产品经理 | 业务主流程参�?|
| `api-contract.yaml` | �?架构专家 | API路由定义参�?|
...
## Dimension 1: 文件存在性与完整�?(File Existence)
...
## Dimension 2: API路由有效�?(API Route Validity)
...
## Dimension 3: 场景覆盖完整�?(Scenario Coverage)

**Question**: Do the scenario tests fully cover all steps in PRD's business main flows?

**Verification Method**:
1. Extract all business main flows from PRD "业务主流�? section
...
## Dimension 4: Fixture类型一致�?(Fixture Type Consistency)
...
| PRD主流�?| 场景测试文件 | 总步�?| 已覆�?| 覆盖�?| 状�?|
|-----------|------------|--------|--------|--------|------|
| 溯源全流�?| marketing-to-trace.test.ts | 6 | 6 | 100% | �?|
| 订单退货流�?| order-return.test.ts | 4 | 3 | 75% | ⚠️ |
...
| 测试文件 | �?| 字段 | Schema类型 | Fixture�?| 状�?|
|---------|-----|------|-----------|----------|------|
| user.test.ts:12 | users | role | TEXT | "admin" | �?|
| product.test.ts:8 | products | price | INTEGER | "free" | �?类型不匹�?|
...
|------|------|---------|-----------|
| 文件存在�?| [PASS/FAIL] | [N] | [N] |
| API路由有效�?| [PASS/FAIL] | [N] | [N] |
| 场景覆盖完整�?| [PASS/FAIL] | [N] | [N] |
| Fixture类型一致�?| [PASS/FAIL] | [N] | [N] |

## ERROR 问题清单（必须修复）

### ERROR #1: [标题]
**维度**: [文件存在�?API路由/场景覆盖/Fixture类型]
**文件**: [路径:行号]
**描述**: [问题描述]
**影响**: [对Stage3的影响]
**修复建议**: [具体修复方案]
...
- **PASS**: �?ERROR 级别问题。WARNING 可记录但通过�?
...
**MUST DO:**
- Check all five dimensions (�?v2.5 视觉维度)
...
- **Visual baseline directory structure**: 截图基线按 `projects[].name` 自动分目录（chromium-headless / chromium-headed / mobile-chrome / mobile-safari），完整目录结构见 `kf-mvp-playwright-infra` Section 3。审查时必须以该结构为准，不能按旧版扁平文件名匹配。
