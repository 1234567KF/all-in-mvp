...
- [ ] **Mock-后端一致性**：7个维度全部匹配
- [ ] 无 P0/P1 Bug 遗留
- [ ] **E2E覆盖率达标**：总用例数 ≥ 70（全量模式）
- [ ] **登录流程覆盖**：至少 12 个用例（每角色+错误路径+改密+找回密码+退出）
- [ ] **菜单导航覆盖**：至少 5 个用例（完整性+权限+跳转+404+面包屑）
- [ ] **CRUD闭环覆盖**：每个模块至少 7 个用例（创建成功/失败+列表+详情+编辑+删除+取消删除）
- [ ] **工作流测试覆盖**：至少 21 个用例（管理员8+销售7+渠道6）
- [ ] **数据权限覆盖**：每个角色至少 1 个权限校验用例
