...
### L4-L5: Playwright 浏览器测试

> **配置由 `kf-mvp-playwright-infra` 统一管理**：`playwright.config.ts` 模板（headed/headless 双 project、CI 集成、截图基线、浏览器实例池）不在本技能重复，请参考 `kf-mvp-playwright-infra` 技能。

**L4/L5 在 5 层测试模型中的角色**:

| 层级 | 模式 | 用途 | 运行方式 |
|------|------|------|---------|
| L4 | Headed (有头) | 本地人工验证，可见浏览器窗口 | `npx playwright test --project=chromium-headed` |
| L5 | Headless (无头) | CI 自动化回归 | `npx playwright test --project=chromium-headless` |

**L4 有头浏览器测试示例**（测试逻辑 — 配置见 infra 技能）:
...
> Playwright API 用法（locator 策略、waitFor、网络拦截等）详见 `kf-mvp-test-e2e` 的「Playwright API 最佳实践」章节。
