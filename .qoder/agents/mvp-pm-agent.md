...
> **原则**：PRD 不能只描述列表展示字段，必须定义交互行为。以下模板用于每个列表类功能描述中。

每个列表类功能需求，PRD 中必须附以下标准交互表：

| 交互能力 | 是否支持 | 规则说明 |
|---------|:------:|---------|
| 分页 | ✅/— | 默认条数、最大条数 |
| 关键词搜索 | ✅/— | 搜索范围（字段名）、匹配方式（模糊/精确） |
| 条件筛选 | ✅/— | 筛选项、数据来源、单选/多选 |
| 排序 | ✅/— | 默认排序字段和方向 |
| 批量操作 | ✅/— | 支持的操作类型 |

**强制规则**：
- 表格中的 ✅ 项必须在「验收标准」中有对应的测试场景
- 标记为 — 的项需在「不做」清单中注明原因
- 缺少此表的列表类功能需求，Domain Expert 可拒绝拆分该模块
