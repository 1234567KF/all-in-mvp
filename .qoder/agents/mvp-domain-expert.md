...
> **原则**：PRD 只描述展示字段时，Domain Expert 必须主动展开隐含交互子 AC。不能等 PRD 定义了才覆盖。

列表类端点（GET 分页查询）的验收标准必须包含以下子项：
...
#### 异常路径
- [ ] XX.7 pageSize 超限 → 400
- [ ] XX.8 非授权角色 → 403
- [ ] XX.9 无 token → 401
```

**判定**：Search & Filter 子项缺失 ≥ 2 项 → AC 不完整，标记为 `INCOMPLETE_AC` → grill 审查时阻塞。
